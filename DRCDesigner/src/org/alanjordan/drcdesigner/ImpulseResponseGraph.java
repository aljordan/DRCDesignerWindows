...
public class ImpulseResponseGraph extends JPanel {
    ...
    private void drawGrid(Graphics2D g) {
        ...
        g.drawString("Amplitude (normalized)", 6, graphYStart - 2);
    }
    ...
    private void drawCurves(Graphics2D g) {
        ...
        drawCurvesPeakCentered(g, maxSamples, windowMs);
    }

    private void drawCurvesPeakCentered(Graphics2D g, int maxSamples, int windowMs) {
        ...
        int measuredLeftPeak = findPeakIndex(measuredLeft);
        int measuredRightPeak = findPeakIndex(measuredRight);
        int predictedLeftPeak = findPeakIndex(predictedLeft);
        int predictedRightPeak = findPeakIndex(predictedRight);

        double sharedPeak = 0.0;
        sharedPeak = Math.max(sharedPeak, findPeakAround(measuredLeft, measuredLeftPeak, preWindowSamples, postWindowSamples));
        sharedPeak = Math.max(sharedPeak, findPeakAround(measuredRight, measuredRightPeak, preWindowSamples, postWindowSamples));
        sharedPeak = Math.max(sharedPeak, findPeakAround(predictedLeft, predictedLeftPeak, preWindowSamples, postWindowSamples));
        sharedPeak = Math.max(sharedPeak, findPeakAround(predictedRight, predictedRightPeak, preWindowSamples, postWindowSamples));
        if (sharedPeak <= 0.0 || !Double.isFinite(sharedPeak)) {
            sharedPeak = 1.0;
        }

        if (showBeforeLeft) {
            drawCurvePeakCentered(g, measuredLeft, BEFORE_LEFT_COLOR, 1.4f, measuredLeftPeak, preWindowSamples, postWindowSamples, sharedPeak, windowMs);
        }
        if (showBeforeRight) {
            drawCurvePeakCentered(g, measuredRight, BEFORE_RIGHT_COLOR, 1.4f, measuredRightPeak, preWindowSamples, postWindowSamples, sharedPeak, windowMs);
        }
        if (showAfterLeft) {
            drawCurvePeakCentered(g, predictedLeft, AFTER_LEFT_COLOR, 2.0f, predictedLeftPeak, preWindowSamples, postWindowSamples, sharedPeak, windowMs);
        }
        if (showAfterRight) {
            drawCurvePeakCentered(g, predictedRight, AFTER_RIGHT_COLOR, 2.0f, predictedRightPeak, preWindowSamples, postWindowSamples, sharedPeak, windowMs);
        }
    }

    private void drawCurvePeakCentered(Graphics2D g,
                                       double[] samples,
                                       Color color,
                                       float strokeWidth,
                                       int peakIndex,
                                       ...
                                       double sharedPeak,
                                       int windowMs) {
        if (samples == null || samples.length < 2 || peakIndex < 0) {
            return;
        }
        ...
        for (int sampleIndex = start; sampleIndex <= end; sampleIndex++) {
            double relativeTimeMs = (1000.0 * (sampleIndex - peakIndex)) / sampleRateHz;
            if (relativeTimeMs < (-windowMs * PRE_PEAK_RATIO) || relativeTimeMs > (windowMs * POST_PEAK_RATIO)) {
                continue;
            }

            double normalized = samples[sampleIndex] / sharedPeak;
            if (normalized > 1.0) {
                normalized = 1.0;
            }
            ...
            double x = translateCenteredMsToX(relativeTimeMs, windowMs);
    ...
    private double findPeakAround(double[] samples, int centerIndex, int preWindowSamples, int postWindowSamples) {
        if (samples == null || samples.length == 0 || centerIndex < 0) {
            return 0.0;
        }

        int start = Math.max(0, centerIndex - preWindowSamples);
        int end = Math.min(samples.length - 1, centerIndex + postWindowSamples);
        double peak = 0.0;
    ...
    private void drawDisclaimer(Graphics2D g) {
        String disclaimer = "Time-domain preview, peak-centered and normalized to shared peak in selected window.";
        Font originalFont = g.getFont();
